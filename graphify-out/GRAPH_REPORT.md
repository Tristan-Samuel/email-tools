...
## Corpus Check
- 36 files · ~49,691 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 778 nodes · 1833 edges · 35 communities (28 shown, 7 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 44 edges (avg confidence: 0.76)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9d7ef02b`
...
1. `EmailStore` - 145 edges
2. `get_store()` - 51 edges
3. `require_login()` - 46 edges
4. `create_app()` - 40 edges
5. `GroqClient` - 40 edges
6. `AiClient` - 39 edges
7. `GeminiClient` - 31 edges
8. `get_ai_client()` - 30 edges
9. `build_email_record()` - 26 edges
...
Cohesion: 0.06
Nodes (7): EmailStore, Replace stored IMAP ciphertext. Return True if a row was updated., Mark queued/running jobs cancelled. Return cancelled job ids., Insert a queued job and return its id., Store a hashed account password for this email-tools user., Return the stored app-password hash, or '' if none set., Release user_moved after lock exceptions (new inbound, sent reply, snooze…
...
Cohesion: 0.06
Nodes (113): accounts(), accounts_add(), accounts_delete(), accounts_load_older(), accounts_sync(), accounts_sync_all(), accounts_update_folders(), accounts_update_password() (+105 more)

### Community 3 - "store.py"
Cohesion: 0.29
Nodes (4): test_ai_confirm_tag_column(), test_marketing_offer_rule_not_seeded(), test_school_tag_prevents_marketing_hide(), test_initialize_fresh_db()
...
Cohesion: 0.07
Nodes (46): create_app(), _credential_encryption_key(), _load_env_file(), _load_secret_key(), Flask, Path, Fill os.environ from `.env` without overriding vars already set., _guess_imap_port() (+38 more)
...
### Community 27 - "test_gemini_client.py"
Cohesion: 0.50
Nodes (3): object, Tests for GeminiClient JSON parsing., test_analyze_emails_batch_parses_items()

### Community 28 - "compact_for_llm"
Cohesion: 0.19
Nodes (8): Return True if AI decides this email should receive the given tag., Return True when email is commercial/promotional bulk mail worth hiding., Return a short reply draft the user can copy or open via mailto:., compact_for_llm(), Shared text compaction for LLM prompts — no imports from provider clients., Strip URLs and tracking noise before sending text to Groq., test_compact_for_llm_strips_urls_keeps_labels(), test_compact_for_llm_truncates_after_stripping()
...
Cohesion: 0.07
Nodes (56): _persist_email_analysis(), build_digest(), build_email_record(), build_important_items(), choose_category(), clean_summary_line(), compact_for_llm(), compute_thread_id() (+48 more)

### Community 34 - "ai_client.py"
Cohesion: 0.13
Nodes (22): is_fatal_auth_error(), is_rate_limit_error(), is_unreachable(), Unified AI client: Gemini primary, Groq fallback., is_fatal_gemini_auth_error(), is_gemini_unreachable(), is_rate_limit_error(), is_tpd_exhausted_error() (+14 more)
...
- **Why does `EmailStore` connect `EmailStore` to `summary.py`, `store.py`, `._job_from_row`, `create_app`, `.enable_default_folders`, `Connection`, `._deserialize_row`, `.apply_all_manual_tags`, `AiClient`, `.update_email_analysis`, `.append_job_log`, `.bump_verification_attempts`?**
  _High betweenness centrality (0.303) - this node is a cross-community bridge._
- **Why does `GroqClient` connect `GroqClient` to `routes.py`, `ai_client.py`, `.analyze_emails_batch`, `create_app`, `AiClient`, `test_ai_client.py`, `compact_for_llm`?**
  _High betweenness centrality (0.076) - this node is a cross-community bridge._
- **Why does `create_app()` connect `create_app` to `EmailStore`, `summary.py`, `routes.py`, `GroqClient`, `email_parser.py`?**
